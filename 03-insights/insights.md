# Insights

Un dato es "OPPO es #4 con 11%". Un insight es qué significa eso para lo que hay que construir.

Fuentes: F1–F4 notas y charla · F5 brief oficial · F6 deck de OPPO · F7 rúbrica de jurados · F8 lluvia de ideas del equipo · F9 Q&A con OPPO · F11 encuesta primaria (n=65).

**Confianza.** Alta = se sostiene con las fuentes. Media = interpretación razonable. Baja = hipótesis nuestra, validar antes de construir encima.

---

## El insight principal (para abrir el pitch)

> **El universitario colombiano teme no capturar el momento tal cual lo vive. Fotografiar no es documentar — es apropiarse de algo que se le escapa, en sus propios términos: "tener yo la capacidad de guardar ese momento como quiera, cuando quiera", "ser yo", "hacer el momento tuyo". El miedo no es no tener qué fotografiar; es que la foto no sea fiel a lo que sintió, y esa versión desdibujada termine reemplazando el recuerdo real.**

Este es el insight humano, construido solo con la pregunta de motivación (Q2) y la pregunta abierta de "Make Your Moment" (Q6) — sin mezclar todavía nada de OPPO:

- **Target:** el universitario colombiano.
- **Verdad (comportamiento):** fotografiar no es documentar, es apropiarse — 82% guarda el recuerdo para sí mismo, no para compartirlo (Q2, I13).
- **Cómo lo dicen ellos mismos (Q6, verbatims):** control y autoría — *"tener yo la capacidad de guardar ese momento de la forma que quiera, cuando quiera y en el lugar que quiera"*, *"libertad de escoger qué constituye un instante memorable para mi vida"*, *"tener poder de hacer lo que quiera"*, *"controlar mis experiencias"*. Fidelidad a lo vivido — *"una foto donde de verdad se vea las emociones"*, *"el lograr mantener una sensación, un ambiente y una acción encapsuladas en un momento"*, *"vivir el momento como yo creo"*. E identidad — *"ser yo"*, *"vivir tal cual soy"*, *"hacer el momento tuyo"*, *"algo que es mío, que me representa o que me identifica"*.
- **Dolor (sentimiento):** **teme no capturar el momento tal cual lo vive** — que la foto no le haga justicia a lo que sintió, y ese registro desdibujado termine siendo el recuerdo que le queda, en vez de la experiencia real. No es un dato de mercado, es una ansiedad humana: la de que lo vivido y lo capturado no coincidan.

**Cómo se conecta con OPPO (un paso aparte, no el insight en sí):** si el miedo es que la cámara no capture el momento tal cual se vivió, entonces la pregunta que le importa al universitario no es "¿esta cámara es buena?" sino "¿esta cámara va a ser fiel a lo que yo sentí, o me lo va a distorsionar?". Ahí es donde entra la marca: de quienes conocen a OPPO (74%), **42% duda de que su cámara esté a la altura de ese momento** (I2) — es decir, casi la mitad no confía en que OPPO resuelva exactamente ese miedo. OPPO mismo, sin que se le preguntara, describe su marca en los mismos términos de identidad: *"un aliado, un compañero, parte de tu estilo de vida"* (F9, I11) — coincide con lo que el universitario ya busca, pero todavía no le cree que lo cumpla.

**Por qué esto es lo que abre el pitch:** conecta los tres pesos más grandes de la nota — el 25% de insights (el humano, con dato propio), el 20% de diferenciación (nadie más va a tener esta cifra) y el 15% de relevancia Reno (la duda está justo en la cámara). Y es la razón de fondo de la mecánica: si fotografiar es apropiarse de un momento, el reto no puede pedir publicar como condición de entrada — tiene que darle a la persona el control primero (el avatar es suyo, a su ritmo) y dejar que compartir sea una opción, no el precio. Eso es "Tu Reno Wrapped" (`../04-ideacion/mecanica-reto-wrapped.md`).

---

## I1 — OPPO no tiene un problema de fotografía. Tiene un problema de identidad

**Observación.** El equipo lo detectó solo, mirando el Instagram de la marca: *"se han adaptado tanto, tanto a las tendencias, que no tienen identidad"* — el feed salta del lanzamiento K-pop *"súper morado y súper futurista"* a contenido del Mundial y después a *"cosas súper diferentes"* (F8). El deck lo confirma sin querer: en 41 páginas conviven la UEFA Champions League, Kaká, Luis Díaz, el fútbol y La Guajira con Pop Camera, estética Y2K, planetas cromados y un Bubble kawaii con un panda morado (F6).

**Tensión.** El equipo formuló el diagnóstico comparado que ninguna diapositiva de OPPO tiene: *"los de iPhone buscan estatus, los de Huawei buscan tecnología"* — y entonces, ¿los de OPPO qué buscan? Esa frase queda sin terminar en la grabación. **Ese blanco es el problema del reto.**

**Implicación.** La pregunta oficial (F5) no menciona la fotografía: pregunta cómo comunicarse *"de una forma auténtica"*. La palabra es auténtica, y una marca que cambia de piel cada campaña no puede serlo. La propuesta ganadora probablemente no es una campaña — es **decidir qué es OPPO para un universitario colombiano** y que la campaña sea la consecuencia. Eso responde el 25% de insights y el 20% de diferenciación al mismo tiempo.

**Resolución concreta (`04-ideacion/mecanica-reto-wrapped.md`).** En vez de que la marca defina su identidad y se la imponga al estudiante, la mecánica invierte el problema: el avatar de cada participante se reconstruye solo, a partir de sus propias fotos, sin comparaciones con embajadores ni famosos (decisión D4, `05-decisiones/bitacora-decisiones.md`) — es 100% personal. La marca deja de saltar sin identidad porque deja de ser ella la que tiene que decidir sola quién es — el momento de identidad se lo queda cada usuario, y OPPO es la herramienta que lo hizo posible, no el protagonista del resultado.

**Confianza:** alta.

---

## I2 — Nadie le cree a la cámara, y la marca tiene la prueba guardada

**Observación.** *"Ahorita nadie cree que OPPO sea capaz de tomar las fotos que dicen que toman"* (F8). Y el contraste que hicieron: Apple publica las fotos tomadas con el teléfono, *"tomas cinematográficas locas… uno dice: yo quisiera tener esta cámara"*, mientras que en el Instagram de OPPO *"nada que me diga: esto somos nosotros, esto es fotografía"*.

**Lo que el equipo no vio.** En el propio deck de OPPO, página 7: **"Co-developed with HASSELBLAD"**. Hasselblad es la marca de las cámaras que fueron a la Luna. También: 65+ patentes concedidas, Official Partner de la UEFA Champions League (F6). La marca tiene credenciales duras y **no las está usando con este público**.

> Cuidado: en el deck, Hasselblad aparece en la diapositiva del **Find N6**, no del Reno 16. Antes de ponerlo en una diapositiva hay que confirmar si la coingeniería aplica a la serie Reno. Sigue sin respuesta (P1).

**Tensión.** La credibilidad no se arregla afirmando. Se arregla mostrando. El equipo listó las tres formas de comparar la calidad de un teléfono: *"uno, que lo vas a comprar y lo comparás en la tienda; dos, que alguien te lo muestre; o que en un entorno donde vos mantenés se use"* (F8). Solo la tercera está al alcance de una campaña.

**Implicación.** Cualquier mecánica que ponga el teléfono en las manos de un estudiante durante un rato resuelve más que toda la pauta. Y encaja exactamente con el recurso que OPPO ofrece: *"apoyo con producto: sí para intercambio y exhibiciones"* (F6, p. 39).

**Validado con encuesta propia, n=65 (F11).** 74% de los encuestados sí conoce la marca OPPO — más de lo que el equipo asumía. Pero de ese 74%, **42% duda de que su cámara capture los momentos que le importa guardar**. La brecha de credibilidad ya no es una cita de seis personas: es una cifra propia, citable en el pitch. Ver `../02-problema/resultados-encuesta.md`.

**Y no es solo la muestra del equipo.** En foros de tecnología internacionales, usuarios independientes describen el marketing de cámara de OPPO como "exagerado" y cuestionan si las reseñas de YouTubers están sesgadas — la misma brecha de credibilidad, en otro idioma y otro mercado (`investigacion-secundaria.md`).

**Confianza:** alta — ahora con dato primario propio, no solo cualitativo.

---

## I3 — El territorio ya está escrito en la propia diapositiva de OPPO

**Observación.** Página 28 del deck, debajo de la foto del Bubble, la copy oficial dice:

> **"Vista trasera ultra nítida. La foto perfecta tomada por tu amigo."**

Y la página 27: *"Selfies traseras. Sin dudas. Solo perfección."*

**Tensión.** El que toma la foto del grupo nunca sale en ella. Es la carencia más cotidiana de la vida universitaria y nadie la nombra. OPPO fabricó un producto que la resuelve y le escribió la línea, pero la trató como especificación técnica en vez de como territorio.

**Implicación.** *"Que no falte nadie"* no es un concepto que haya que pegarle al producto: sale del producto, y de la copy de la marca. Eso es justo lo que califica el 15% de "conexión natural con Reno y el territorio de la fotografía", y es lo contrario de lo que OPPO dijo que no quiere: *"empieza con un insight, no con un formato"* (F6, p. 41).

Se conecta con lo que el equipo dijo del país: *"el colombiano es muy emocional, es una cultura muy de estar con gente"* (F8).

**Confianza:** alta — la cita es literal del deck.

---

## I4 — Un evento de fotografía repele a quien no es fotógrafo

**Observación.** El momento más útil de la lluvia de ideas fue una pregunta y un silencio:

> — *"¿Alguno aquí iría a un evento de fotografía?"*
> — *"No, porque no me interesa."*

Nadie levantó la mano (F8). Seis universitarios del target, y cero.

**Tensión.** OPPO califica con 20% la *"motivación genuina para que los estudiantes se involucren"*. Un concurso o un evento de fotografía convoca a quien ya se considera fotógrafo: una minoría diminuta en cualquier campus. El resto mira, y mirar no es participar.

**Implicación.** La mecánica no puede pedir fotografiar. Tiene que pedir algo que la gente ya hace, y que resulte ser fotografía. Si la propuesta se puede describir como "un concurso de fotos", el propio equipo ya demostró que no iría.

**Y el concurso no es obligatorio.** En el deck es una de ocho burbujas del 360, sin jerarquía (F6, p. 38), y la última diapositiva dice *"la solución está abierta"*. Lo que quedó en las notas como "concurso muy fuerte" (F1) parece haber sido énfasis del expositor, no requisito.

**Confianza:** alta.

---

## I5 — El equipo está mirando un campus que no es el del reto

**Observación.** Autocrítica textual del grupo: *"nosotros estamos muy sesgados por el entorno en el que estamos. Estamos literalmente rodeados de Apple, pero no sabemos las otras universidades: estudiantes de la Valle, de la Santiago"* (F8). Y: *"somos universitarios, pero estamos en un sector donde realmente la gente tiene plata"*.

**Tensión.** El reto dice "estudiantes universitarios colombianos", no "estudiantes de universidad privada". Un Reno 16 de $2.299.000–$2.999.000 es gama media-alta: caro para el bolsillo del equipo comparado con un iPhone, pero **razonable frente a un iPhone** para quien nunca consideró comprar uno.

**Implicación doble.** Primero: la investigación tiene que salir de la burbuja — Univalle, Santiago, públicas. Segundo, y más valioso: **los otros equipos del reto viven en la misma burbuja.** Si ustedes son los únicos con datos de fuera, ese contraste no es un dato más, es el diferenciador de la presentación. Y el brief exige investigación primaria con nombre propio (F5).

Matiz que el propio equipo puso: la relación estrato → marca no es directa, *"hay muchas personas que se endeudan por conseguirse un iPhone"*. Lo que manda es presión social, no ingreso.

**Confianza:** alta en el sesgo, alta en la instrucción de investigación.

---

## I6 — Ya existe un perfil que sí compra OPPO, y no es el que el equipo imaginaba

**Observación.** El único caso real que apareció en la sesión: una amiga con OPPO. Estudia **Humanidades**. Lo conoció **por una propaganda** de resistencia — *"lo tiraban desde no sé dónde y eso resistía"*. Se quedó por la cámara. *"Le gusta mucho tomar fotos tipo con cámara, no profesional, sino como las fotos que se ven vintage"* (F8).

**Tensión.** Es exactamente el perfil que Pop Camera fabrica: cámara digital, película instantánea, fuga de luz (F6, p. 19). No es el fan de K-pop ni el hincha de Luis Díaz. Es alguien que quiere que sus fotos **se vean de una manera**, no que se vean perfectas.

**Implicación.** Hay un usuario real y describible: estudiante de humanidades, artes o carreras creativas, con gusto por lo retro y lo imperfecto. Es un caso, no una muestra — pero da la hipótesis que la encuesta debe confirmar o tumbar en las primeras horas.

**Confianza:** media — es n=1. Barato de validar.

---

## I7 — Apple y Samsung compiten por la foto perfecta; ese terreno está tomado

**Observación.** Las funciones estrella del Reno no son de nitidez, son de **estilo**: Pop Camera degrada la imagen a propósito — grano, fuga de luz, look de cámara vieja (F6, p. 19). Competencia declarada: Apple y Samsung (F2).

**Contra-argumento que ya salió, y hay que responderlo.** *"Hay muchas aplicaciones gratis, eso yo lo puedo hacer en VSCO"* (F8). Es cierto y hay que enfrentarlo de una vez: el diferenciador no es el filtro, es que **está en la cámara nativa**. Cero fricción, cero apps, cero pasos. La diferencia entre editar una foto y que la foto salga así.

**Implicación.** Nombrar el terreno en la propuesta: OPPO no puede ganar la pelea de la nitidez y no debería darla. Puede quedarse con la foto que se siente. Y eso además baja la barrera de I4: el carácter no requiere saber fotografía.

**Confianza:** alta.

---

## I8 — El Bubble es una puerta de entrada barata y está subexplotado

**Observación.** $399.000 — una sexta parte del teléfono. **Funciona también con iPhone** (F1, F2). Salió de una investigación de mercado a la generación Z (F4). OPPO lo llama *"la pieza infaltable de tu outfit"* y lo muestra como llavero, muñequera, correa y collar (F6, p. 26). Están testeando el mercado con pocas unidades (F3).

**Tensión.** Es el único producto del portafolio que un universitario puede comprar, se lleva puesto — o sea que se ve — y entra al bolsillo de alguien que tiene la competencia en la mano. El equipo lo dijo bien: *"es la manera de ingresar"*, *"ingresar por el producto que más diferente es"*.

**El límite, que también dijeron.** OPPO fue claro en que el foco es el celular y la serie Reno completa, no el accesorio. *"El foco 100% no nos podemos desviar, es el celular"* (F8).

**Confirmado por OPPO directamente (F9):** *"nuestra campaña se basó en eso — llamar la atención, el awareness a punta del Bubble... y después conectarlo con el celular."* Ya no es una lectura estratégica del equipo, es la estrategia que OPPO ya corre. Cierra DP2.

**Implicación.** El Bubble como **puerta**, el Reno como **casa**. Es la lectura que sobrevive a las dos restricciones: aprovecha el objeto accesible sin desobedecer el foco. Y hay una idea del equipo que merece rescate — el Bubble como dispositivo de comunicación por fotos entre personas (B2), que conecta con la copy oficial de la marca y con I3.

**Confianza:** alta en los hechos, media en la lectura estratégica.

---

## I9 — El momento no está en el campus

**Observación.** *"De pronto los universitarios no es su momento de estar en la universidad, sino después con los amigos, ese parche"* (F8). Y sobre la generación: *"somos jóvenes, pero recordamos mucho lo que vivimos"*, *"uno en la universidad suele ser muy nostálgico, como querer capturar todo"*.

**Tensión.** Una campaña que se instala solo en el campus busca a la gente donde estudia, no donde vive. La foto que importa no es la de la clase.

**Implicación.** El campus es el **canal de distribución** — ahí se convoca, ahí se ven las carteleras, ahí están los colectivos. Pero la mecánica tiene que activarse afuera: el parche, el bus, la fila, la salida del viernes. Distinguir las dos cosas evita el error de diseñar una activación de pasillo.

**Confianza:** media — es percepción del equipo, va en la encuesta.

---

## I10 — La sostenibilidad está en el brief oficial, no como adorno

**Observación.** El brief oficial (F5) tiene una casilla llamada *"¿Cómo se conecta este reto con la Sostenibilidad?"*, y OPPO la llenó con la Fundación Luis Díaz: acceso a conectividad y herramientas tecnológicas para jóvenes beneficiarios, *"entendiendo la tecnología como un medio para generar nuevas oportunidades"*. En la charla dieron la cifra: 7 de cada 10 jóvenes rurales sin acceso a tecnología conectada, hasta 40% menos oportunidades (F3).

**Implicación.** Que esté en el formulario oficial significa que los evaluadores lo van a buscar, y alimenta el 20% de "viabilidad y potencial de impacto". Además resuelve la contradicción de I1: el eje Luis Díaz deja de ser fútbol y pasa a ser origen y oportunidad, que sí conversa con fotografía.

**Cuidado.** Si se hace mal es *causewashing* y el jurado lo huele. La conexión tiene que ser estructural, no un párrafo al final.

**Confianza:** alta.

---

## I11 — OPPO ya dijo qué identidad quiere, y casi nadie lo va a citar

**Observación.** En una sesión de preguntas y respuestas (F9), el encargado de OPPO definió la marca con una claridad que no está en ninguna diapositiva del deck: *"la visión de la marca es más como un aliado, un compañero, alguien de tu estilo de vida, parte de tu estilo... que esté en todo, porque ustedes no solo estudian, también salen a rumbear, viajan, todo. En todos esos momentos está el celular. Pero muy basado sobre todo en cámara, en fotografía."*

**Tensión.** Esto responde directamente al blanco que dejó I1 ("¿los de OPPO qué buscan?"): buscan ser **estilo de vida**, no herramienta académica ni fandom de una tendencia pasajera. Es la propia marca resolviendo su propio problema de identidad — pero en una llamada de mentoría, no en un documento que los otros equipos necesariamente escucharon con el mismo detalle.

**Implicación.** Citar esta frase casi textual en el pitch es evidencia de que la propuesta nace de lo que OPPO pidió, no de una interpretación libre — y responde a la vez el 25% de insights (viene de la marca) y el 15% de relevancia para Reno.

También en F9: los diferenciadores frente a Apple no son solo la cámara — **diseño** e **IA nativa de productividad** (traducir documentos, escanear, resumir desde el celular, algo que el iPhone no hace nativo). Son material secundario válido para el pitch, pero OPPO fue explícito: *"el foco sea fotografía"* — no vale pivotear la idea central hacia productividad.

**Confianza:** alta — cita directa de OPPO.

---

## I12 — El target real puede ser la burbuja de la que el equipo se quería alejar

**Observación.** F9: *"este celular... está enfocado para estudiantes y es para estratos altos, en realidad, porque no es económico. Entonces es como muy Andes, Sabana, como muy eso."* El alcance pedido es nacional, pero el perfil económico real del comprador coincide con el círculo social del propio equipo (I5).

**Tensión.** El equipo diseñó la encuesta para salir deliberadamente de su burbuja (S1–S8, D2) porque asumía que su sesgo era un riesgo. OPPO acaba de decir que ese círculo **sí es** el target real del producto. No invalida la validación por fuera de la burbuja — sigue siendo el diferenciador frente a otros equipos y el alcance es nacional, no solo su universidad — pero si la muestra de la encuesta se pesa igual que antes, el insight central puede terminar hablando de un segmento que no puede pagar el producto.

**Implicación.** Antes de que la encuesta salga (o apenas salga, si ya lo hizo), el equipo debe decidir explícitamente cómo pesar la muestra: ¿mayoría de estrato alto/universidad privada con el contraste de fuera como matiz, o se mantiene el diseño original 40/60? Ver DP5 en `../05-decisiones/bitacora-decisiones.md`.

**Ya no es hipotético — se confirmó (F11, n=65).** 83% de las respuestas recolectadas son de estrato 4–6. La encuesta terminó hablando sobre todo de la burbuja, no de fuera de ella. Dado lo que dice F9, esto no necesariamente invalida el insight (el target real sí parece ser ese estrato), pero hay que ser honestos en el pitch sobre a quién representa realmente la muestra, y sigue faltando el cruce por universidad (S4) que nunca se recogió.

**Confianza:** alta en la cita y en el sesgo confirmado; media en cómo se resuelve para el pitch — es una decisión de equipo, no un hecho que se resuelva solo.

---

## I13 — La gente fotografía para sí misma, no para publicar

**Observación.** Encuesta primaria, n=65 (F11): a la pregunta de cuál es la motivación principal para tomar una foto, **82% respondió "guardar recuerdos"** y solo **1 persona de 65 (2%)** respondió "compartir y conectar". Documentar experiencias (11%) y capturar una emoción (6%) completan el resto — todas orientadas hacia adentro, no hacia una audiencia.

**Tensión.** Es fácil diseñar un reto asumiendo que a la gente le entusiasma publicar. El dato dice lo contrario: la foto es, ante todo, para uno mismo. Un mecanismo que exija publicar como condición de entrada está pidiendo justo lo que menos motiva a este público.

**Implicación.** Confirma con dato duro lo que S1 sostenía como supuesto. El diseño de "Tu Reno Wrapped" (`../04-ideacion/mecanica-reto-wrapped.md`) ya apunta en la dirección correcta —el avatar es un beneficio personal primero, compartir la tarjeta al final es una opción, no un requisito— y esta cifra es la razón por la que ese orden importa y no es un detalle de diseño menor.

**Confianza:** alta — n=65, pregunta directa y sin ambigüedad en las respuestas.

---

## I14 — El interés real es abrumadoramente social, no parejo

**Observación.** Encuesta primaria, n=65 (F11): al preguntar por el plan ideal de fin de semana, **46% eligió algo centrado en comunidad** (fiesta y festivales de música, cultura urbana), casi el doble que el segundo lugar, deportes extremos/aire libre (26%). Creación de contenido aesthetic (22%) y gaming (6%) completan la lista.

**Tensión.** Es tentador diseñar los cuatro dominios del avatar como si pesaran igual. El dato dice que no: casi la mitad del interés real vive en un solo dominio.

**Matiz que hay que decir si preguntan:** las opciones ofrecidas eran todas de ocio — no se incluyó ninguna alternativa académica. El hallazgo correcto es "dentro de las formas de pasar el tiempo libre, comunidad gana por mucho", no "nadie quiere estudiar" — la encuesta no comparó eso.

**Implicación.** Comunidad debe tener más categorías y más oportunidades de sumar puntos en el reto que los demás dominios (ya reflejado en `../04-ideacion/mecanica-reto-wrapped.md`), y la activación presencial (piloto de esta noche, estación Reno) debería priorizar contextos sociales/grupales sobre individuales.

**Confianza:** alta en el dato, media en la generalización — n=65 de una encuesta con sesgo de muestra ya documentado en S8.

---

## Señales sueltas

- El deck **no menciona inversión en pauta** (F6, p. 39), aunque la charla sí (F3). **Confirmado en F9**: no hay presupuesto de pauta específico para esta estrategia. Un plan que asume presupuesto de medios está asumiendo de más.
- Competidores confirmados por OPPO: **Apple y Samsung**. Huawei quedó fuera de la conversación — perdió acceso a Google Play Services y "se los mataron" (F9).
- Matiz técnico agudo del equipo: la percepción de "mala cámara" puede ser **la pantalla**, no el sensor — *"hay mucha diferencia en sensación visual entre un iPhone normal y un iPhone Pro"* (F8). Si es cierto, el problema se resuelve en cómo se muestran las fotos, no en cómo se toman.
- **Segundo consumidor** (B5): el estudiante tiene iPhone, pero influye en el celular que compran sus papás o abuelos. Territorio grande y sin explorar, aunque se sale del target declarado.
- Una integrante se enteró del lanzamiento **por un influencer**, no por la pauta (F8). Dato pequeño con implicación grande sobre qué canal funciona.
- Nadie del equipo captó el nombre del expositor. La líder del proyecto, según el brief oficial, es **Vansie** (F5).
- La resistencia (IP69, 2 m bajo agua, grado militar) fue lo que enganchó al único caso real de compra que apareció (F8) — y es un ángulo de contenido sin explotar en un país con este clima.
